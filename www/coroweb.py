...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        ...
        if found and (
            ...
            raise ValueError(
                'request parameter must be the last named paramter in function: %s%s' % (fn.__name__, str(sig)))
...
def add_routes(app, moudle_name):
    ...
    '''
    循环出模块的中的方法
    如：['Runoob', '__builtins__', '__cached__', 
            '__doc__', '__file__', '__loader__', '__name__',
             '__package__', '__spec__', 'get', 'get_api']
    '''
    for attr in dir(mod):
